    ...
    def test_program(self, program_lines: List[str]) -> Dict:
        ...
        try:
            ...
            if self.name == "Extension Challenge":
                import random
                # Random starting position (any floor tile in 4x4 room)
                possible_positions = [(x, y) for x in range(4) for y in range(4)]
                ...
                runner.correct_key_pos = (1, 3)  # The key at (1,3) is the correct one
            else:
...
class VaultRunnerGame:
    ...
    def _create_challenges(self) -> List[GameChallenge]:
        ...
        challenges = [
            ...
            GameChallenge(
                ...
                description="Solve a 4x4 map with multiple keys, one door, and one exit. Only one key opens the door. Unknown starting position and direction!",
                world_creator=lambda: self._create_extension_world(),
                start_pos=(2, 2),  # Center of 4x4 room
    ...
    def _create_extension_world(self):
        ...
        # 4x4 orthogonal rectangular room (larger for better exploration)
        for x in range(4):
            for y in range(4):
                world[(x, y)] = 'floor'
        ...
        for y in range(-1, 5):
            ...
            world[(4, y)] = 'wall'   # Right wall
        
        # Multiple keys - only one will work
        world[(0, 0)] = 'key'  # Wrong key
        world[(3, 0)] = 'key'  # Wrong key
        world[(1, 3)] = 'key'  # Correct key (this one opens the door)
